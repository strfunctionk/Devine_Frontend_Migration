...
const CommonChip = ({
  ...
  return (
    <button
      ...
      {icon && (
        <span className="flex items-center justify-center w-20pxr h-20pxr shrink-0 [&>svg]:max-w-full [&>svg]:max-h-full">
          {icon}
        </span>
      )}
      <span className="text-caption1-md">{label}</span>
      {checkFill && icon && (
        <CheckFill className="text-ui-50 absolute -left-4pxr -top-4pxr h-16pxr w-16pxr" />
      )}
      {onRemove && (
        <button
          type="button"
          ...
          onClick={onRemove}
          aria-label="삭제">
          <Cancel className="h-9pxr w-9pxr m-3pxr" />
        </button>
      )}
    </button>
  );
